...
> **공식 문서**: [Configure private connectivity to resources in your VNet](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network)
...
- [10. Step 8: NCC 생성](#10-step-8-ncc-생성)
- [11. Step 9: Private Endpoint Rule 추가](#11-step-9-private-endpoint-rule-추가)
  - [11.1 Private Link Service의 Resource ID 확인](#111-private-link-service의-resource-id-확인)
  - [11.2 Private Endpoint Rule 생성](#112-private-endpoint-rule-생성)
  - [11.3 도메인 이름(Domain Names) 작성 가이드](#113-도메인-이름domain-names-작성-가이드)
- [12. Step 10: Private Endpoint 승인](#12-step-10-private-endpoint-승인)
- [13. Step 11: PE 상태 확인 및 NCC를 Workspace에 연결](#13-step-11-pe-상태-확인-및-ncc를-workspace에-연결)
  - [13.1 Private Endpoint 상태 확인](#131-private-endpoint-상태-확인)
  - [13.2 NCC를 Workspace에 연결](#132-ncc를-workspace에-연결)
  - [13.3 (선택) Network Policy 설정](#133-선택-network-policy-설정)
- [14. Step 12: 연결 검증](#14-step-12-연결-검증)
- [15. 트러블슈팅](#15-트러블슈팅)
- [16. 참고 자료](#16-참고-자료)
...
> Databricks Account Console에서 Network Connectivity Configuration (NCC)을 생성합니다. NCC는 Serverless 컴퓨팅의 네트워크 연결을 관리하는 계정 수준 구성 객체입니다.
...
> **참고 문서**: [NCC 개요](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/#ncc)

---

## 11. Step 9: Private Endpoint Rule 추가

> NCC에 Private Endpoint Rule을 추가하여 Serverless 컴퓨팅에서 Private Link Service를 통해 고객 VNet의 리소스에 접근할 수 있도록 합니다. 이 단계는 공식 문서의 [Step 4: Create a private endpoint](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network#step-4-create-a-private-endpoint)에 해당합니다.

### 11.1 Private Link Service의 Resource ID 확인

Private Endpoint Rule을 생성하려면 **Step 3에서 생성한 Private Link Service의 Resource ID**가 필요합니다.

1. Azure Portal에서 **Private Link services** (검색창에서 "Private link" 검색) 로 이동
2. Step 3에서 생성한 Private Link Service(`pls-databricks`)를 클릭
3. **Overview** 페이지에서 **JSON View** 클릭 (또는 **Properties** 메뉴)
4. 페이지 상단에 표시되는 **Resource ID**를 복사

   Resource ID 형식 예시:
   ```
   /subscriptions/<subscription-id>/resourceGroups/<resource-group-name>/providers/Microsoft.Network/privateLinkServices/<private-link-service-name>
   ```

   실제 예시:
   ```
   /subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/rg-databricks-networking/providers/Microsoft.Network/privateLinkServices/pls-databricks
   ```

> ⚠️ **중요**: 여기서 입력하는 Resource ID는 **최종 대상 리소스(Storage Account, SQL Database 등)의 ID가 아닙니다**. 반드시 **Private Link Service**의 Resource ID를 사용해야 합니다. 이 아키텍처에서 트래픽은 PLS → Load Balancer → Router VM → Azure Firewall을 경유하여 최종 대상에 도달합니다.

### 11.2 Private Endpoint Rule 생성

1. [Account Console](https://accounts.azuredatabricks.net/) > **Security** > **Network connectivity configurations** 클릭
2. Step 8에서 생성한 NCC를 선택
3. **Private endpoint rules** 탭에서 **Add private endpoint rule** 클릭
4. 설정:

   | 설정 | 값 | 설명 |
   |------|-----|------|
   | **Azure resource ID** | Private Link Service의 Resource ID (11.1에서 복사) | PLS의 전체 리소스 경로 |
   | **Domain names** | VNet 리소스의 커스텀 도메인 이름 (11.3 참고) | 최대 **10개**까지 입력 가능 |

5. **Add**
6. Private endpoint rules 목록에서 새로 추가된 규칙의 **Status** 컬럼이 **`PENDING`** 으로 표시되는지 확인합니다. `PENDING` 상태가 되기까지 수 분이 소요될 수 있습니다.
...
#### Step A: 접근 대상 리소스의 FQDN 결정

Serverless 워크로드에서 접근해야 하는 VNet 내부(또는 ExpressRoute를 통한 온프레미스) 리소스의 FQDN을 목록화합니다.

1. Databricks 노트북, Job, SQL Warehouse에서 접근해야 하는 **모든 대상 리소스**를 식별합니다
2. 각 리소스의 **FQDN(정규화된 도메인 이름)** 을 확인합니다

   | 리소스 유형 | FQDN 확인 방법 | FQDN 예시 |
   |-----------|--------------|----------|
   | VNet 내부 VM/서비스 | 해당 VM의 DNS 이름 또는 Private DNS Zone에 등록된 이름 | `api.internal.contoso.com` |
   | 온프레미스 서버 | 조직 내부 DNS에서 관리하는 도메인 | `data.corp.contoso.com` |
   | Azure PaaS (VNet 내부 경유) | Azure Private DNS Zone에 등록된 커스텀 도메인 | `mydb.internal.contoso.com` |

3. PE당 **최대 10개**의 도메인을 등록할 수 있으므로, 10개를 초과하는 경우 **추가 Private Endpoint Rule**을 생성합니다

> ⚠️ **주의**: 여기서 말하는 도메인 이름은 `*.blob.core.windows.net` 같은 Azure 기본 공개 도메인이 아닙니다. 이 아키텍처에서는 PLS → LB → Router VM → Firewall 경로를 통해 접근하므로, **고객이 직접 관리하는 내부 도메인 이름**을 사용합니다.

#### Step B: Load Balancer Backend Pool의 IP 확인

도메인 이름이 resolve 될 대상 IP를 확인합니다. 이 IP는 Load Balancer Backend Pool에 등록된 Router VM의 Private IP입니다.

1. Azure Portal > **Load balancers** > `lb-databricks-internal` > **Backend pools** > `be-router-pool` 클릭
2. Backend Pool에 등록된 **Router VM의 Private IP 주소**를 확인합니다

   예시:
   | VM | Private IP | 가용성 영역 |
   |----|-----------|------------|
   | `vm-router-01` | `10.0.2.68` | Zone 1 |
   | `vm-router-02` | `10.0.2.69` | Zone 2 |

3. 이 IP 주소들이 다음 Step C에서 DNS A 레코드의 대상이 됩니다

#### Step C: Azure Private DNS Zone에 A 레코드 등록

Step A에서 결정한 각 FQDN이 Step B의 Router VM IP로 직접 resolve 되도록 DNS 레코드를 생성합니다.

1. Azure Portal > **Private DNS zones** > **+ Create** (기존 DNS Zone이 있으면 해당 Zone 사용)

   | 설정 | 값 |
   |------|-----|
   | Subscription | Databricks 구독 |
   | Resource group | `rg-databricks-networking` |
   | Name | `internal.contoso.com` (예시 — 조직의 내부 도메인) |

2. **Review + create** > **Create**

3. Private DNS Zone > **+ Record set** 으로 A 레코드를 추가합니다:

   | Name | Type | TTL | Value | 설명 |
   |------|------|-----|-------|------|
   | `api` | A | 300 | `10.0.2.68` | api.internal.contoso.com → Router VM #1 |
   | `db` | A | 300 | `10.0.2.68` | db.internal.contoso.com → Router VM #1 |
   | `storage` | A | 300 | `10.0.2.68` | storage.internal.contoso.com → Router VM #1 |

   > 💡 **HA 구성 시**: 동일 레코드에 Router VM #2의 IP(`10.0.2.69`)도 추가하면 DNS 라운드로빈으로 부하 분산이 가능합니다. 단, 이 아키텍처에서는 LB가 부하 분산을 수행하므로, **Router VM #1의 IP만 등록하는 것이 일반적**입니다.

4. Private DNS Zone을 **Databricks VNet에 링크**합니다:
   - Private DNS Zone > **Virtual network links** > **+ Add**

   | 설정 | 값 |
   |------|-----|
   | Link name | `link-vnet-databricks` |
   | Virtual network | `vnet-databricks` |
...
> ⚠️ **DNS 제약 사항 (반드시 확인)**:
> - **DNS Chasing 미지원**: CNAME 레코드 체인을 따라가는 방식은 지원되지 않습니다. 반드시 **A 레코드로 직접** IP를 지정하세요.
> - **DNS Redirect 미지원**: DNS redirect도 지원되지 않습니다.
> - 예를 들어, `api.contoso.com → CNAME → api.internal.contoso.com → A → 10.0.2.68` 과 같은 CNAME 체인은 **동작하지 않습니다**. `api.contoso.com → A → 10.0.2.68` 처럼 직접 A 레코드를 사용해야 합니다.

#### Step D: DNS 확인(Resolution) 검증

DNS 레코드가 올바르게 설정되었는지 검증합니다. Router VM에 SSH로 접속하여 확인합니다.

```bash
# 각 도메인의 DNS 확인 테스트
nslookup api.internal.contoso.com
nslookup db.internal.contoso.com
nslookup storage.internal.contoso.com

# 또는 dig 명령 사용
dig api.internal.contoso.com +short
# 기대 결과: 10.0.2.68

dig db.internal.contoso.com +short
# 기대 결과: 10.0.2.68
```

각 도메인이 Router VM의 Private IP로 올바르게 resolve 되는지 확인합니다. resolve 되지 않는 경우:
- Private DNS Zone의 A 레코드가 정확한지 확인
- Private DNS Zone이 `vnet-databricks`에 링크되었는지 확인
- VNet의 DNS 설정이 Azure 기본 DNS(`168.63.129.16`)를 사용하는지 확인

#### Step E: Azure Firewall에 대상 FQDN 규칙 추가

Router VM이 트래픽을 수신한 뒤 Azure Firewall을 통해 **최종 대상 리소스**에 도달할 수 있도록, Step 7의 Firewall Application Rule에 최종 대상의 FQDN을 추가합니다.

1. Azure Portal > **Firewall Policies** > `afwp-hub`
2. **Application Rules** > 기존 Rule collection(`rc-databricks-allowed`) 편집
3. 최종 대상 리소스의 FQDN을 규칙에 추가합니다:

   | 이름 | Source | Protocol | Destination (FQDN) | 설명 |
   |------|--------|----------|-------------------|------|
   | `Allow-Internal-API` | `10.0.2.64/26` | `Https:443` | `api-server.real-target.com` | 실제 API 서버 |
   | `Allow-Internal-DB` | `10.0.2.64/26` | `Mssql:1433` | `mydb.database.windows.net` | 실제 DB 서버 |

   > **핵심**: Domain names 필드에 입력한 FQDN은 PLS→LB→Router VM까지의 라우팅에 사용되고, Firewall 규칙의 FQDN은 **Router VM에서 최종 대상까지의 접근 허용**에 사용됩니다.

4. **Save**

#### Step F: Private Endpoint Rule에 도메인 이름 입력

위 Step A~E가 완료되었으면, 11.2의 Private Endpoint Rule 생성 시 **Domain names** 필드에 Step A에서 결정한 FQDN을 입력합니다.

1. Account Console > NCC > **Add private endpoint rule** (또는 기존 규칙 편집)
2. **Domain names** 필드에 도메인 이름을 입력합니다 (콤마 또는 줄바꿈으로 구분):

   ```
   api.internal.contoso.com
   db.internal.contoso.com
   storage.internal.contoso.com
   ```
...
```
Step A: 접근 대상 FQDN 결정
    ↓
Step B: Router VM의 Private IP 확인
    ↓
Step C: Private DNS Zone에 A 레코드 등록 (FQDN → Router VM IP)
    ↓
Step D: DNS 확인 검증 (nslookup / dig)
    ↓
Step E: Azure Firewall에 최종 대상 FQDN 규칙 추가
    ↓
Step F: NCC Private Endpoint Rule의 Domain names 필드에 FQDN 입력
```

> **참고 문서**: [Configure private connectivity to resources in your VNet — Step 4](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network#step-4-create-a-private-endpoint)

<details>
<summary><strong>📌 참고: Azure PaaS 리소스에 직접 Private Endpoint를 생성하는 경우</strong></summary>

Azure PaaS 리소스(Storage, SQL 등)에 **Router VM/Firewall을 경유하지 않고 직접** Private Endpoint를 생성하는 경우에는, Private Link Service가 아닌 **대상 Azure 리소스의 Resource ID**와 **Sub-resource ID**를 사용합니다. 이 시나리오에 대한 자세한 내용은 [Configure private connectivity to Azure resources](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/serverless-private-link)를 참고하세요.

#### 자주 사용되는 Sub-resource ID (직접 PE 시나리오)
...
## 12. Step 10: Private Endpoint 승인

> Databricks에서 생성한 Private Endpoint 연결 요청을 **Private Link Service** 측에서 승인합니다. 승인 전까지 연결은 활성화되지 않습니다. 이 단계는 공식 문서의 [Step 5: Accept the private endpoint on your resource](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network#step-5-accept-the-private-endpoint-on-your-resource)에 해당합니다.

1. Azure Portal에서 **Private Link Center**로 이동 (검색창에서 "Private Link" 검색)
2. 왼쪽 사이드바에서 **Private Link services** 클릭
3. Step 3에서 생성한 Private Link Service(`pls-databricks`)를 선택
4. 왼쪽 사이드바의 **Settings** 아래에서 **Private endpoint connections** 클릭
5. Databricks가 생성한 Private Endpoint를 확인합니다:
   - 연결 상태(Connection state)가 **Pending** 으로 표시됩니다
   - Description에 Databricks 관련 정보가 포함되어 있습니다
6. 승인할 endpoint의 **체크박스** 선택
7. 상단의 **Approve** 버튼 클릭
8. 확인 대화상자에서 **Yes** 클릭
9. 승인 후 연결 상태가 **Approved**로 변경됩니다

> ⚠️ **참고**: 연결이 완전히 설정되기까지 약 **10분**이 소요될 수 있습니다.

> **참고 문서**: [Private Link Service — Private endpoint connections 관리](https://learn.microsoft.com/en-us/azure/private-link/manage-private-endpoint)

---

## 13. Step 11: PE 상태 확인 및 NCC를 Workspace에 연결

> Private Endpoint가 정상적으로 설정되었는지 확인한 후, NCC를 Workspace에 연결합니다. 이 단계는 공식 문서의 [Step 6](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network#step-6-confirm-private-endpoint-status)과 [Step 7](https://learn.microsoft.com/en-us/azure/databricks/security/network/serverless-network-security/pl-to-internal-network#step-7-attach-the-ncc-to-one-or-more-workspaces)에 해당합니다.

### 13.1 Private Endpoint 상태 확인

1. [Databricks Account Console](https://accounts.azuredatabricks.net/)로 이동
2. **Security** > **Network connectivity configurations** 클릭
3. 해당 NCC를 선택
4. **Private endpoint rules** 탭에서 Private Endpoint의 **Status** 컬럼을 확인합니다
5. 상태가 **`ESTABLISHED`** 로 변경되었는지 확인합니다
   - 페이지를 새로고침하여 최신 상태를 확인합니다
   - `PENDING` → `ESTABLISHED`로 변경되기까지 약 10분이 소요될 수 있습니다

| 상태 | 의미 | 조치 |
|------|------|------|
| `PENDING` | Azure 측에서 승인 대기 중 | Step 10의 승인 절차를 수행하세요 |
| `ESTABLISHED` | 연결 완료 — 정상 | 다음 단계로 진행 |
| `REJECTED` | Azure 측에서 거부됨 | 관리자에게 승인 요청, 필요 시 PE Rule 삭제 후 재생성 |

### 13.2 NCC를 Workspace에 연결

Private Endpoint 상태가 `ESTABLISHED`로 확인된 후, NCC를 Workspace에 연결합니다.

1. Account Console 사이드바에서 **Workspaces** 클릭
2. 대상 Workspace 이름 클릭
3. **Update workspace** 클릭
4. **Network connectivity configurations** 드롭다운에서 Step 8에서 생성한 NCC 선택
   > NCC가 보이지 않으면 Workspace와 NCC의 Azure 리전이 동일한지 확인하세요.
5. **Update**
6. **변경 사항이 적용될 때까지 약 10분 대기**
7. NCC를 적용하려는 모든 Workspace에 대해 **2~5 단계를 반복**합니다

> 💡 **팁**: NCC는 리전 단위 객체이므로, **동일 리전의 Workspace에만** 연결할 수 있습니다.

### 13.3 (선택) Network Policy 설정
...
### 15.1 일반적인 문제 및 해결 방법

| 증상 | 가능한 원인 | 해결 방법 |
|------|-----------|----------|
| Private Endpoint 상태가 `PENDING`에서 변하지 않음 | Private Link Service 측에서 승인하지 않음 | Azure Portal > Private Link Center > Private Link services > `pls-databricks` > Private endpoint connections에서 승인 |
| Private Endpoint 상태가 `REJECTED` | Private Link Service 관리자가 거부함 | 관리자에게 승인 요청, 필요 시 PE Rule 삭제 후 재생성 |
...
| 도메인 이름으로 접근 실패 | DNS 설정 오류 또는 DNS chasing 사용 | 도메인이 Backend Pool IP로 직접 resolve 되는지 확인, CNAME 체인 제거 |

### 15.2 Router VM 진단 명령
